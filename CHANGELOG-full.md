...
- **The setup wizard can be run again** — Settings (gear) → Setup → *Run setup
  wizard*.

  It hid itself once `setup_complete` was set and nothing ever cleared that
  flag, so an existing install had no way back short of editing the database or
  reinstalling. That was a poor answer for the two steps most worth
  revisiting: the hardware summary, and the entity naming scheme — the one
  setting that cannot be changed casually afterwards, because Home Assistant
  fixes an entity id at creation.

  **Nothing is reset.** Every step reads live state, so reopening shows what is
  configured now rather than an empty form; the gateway, prefix and credentials
  are untouched. Reopening is audited separately from completing and skipping,
  so a support conversation can tell the three apart.
...
- **The red icons in the top bar were overstating the problem.** The globe went
  red for any poll status other than `running`/`ok`, which put
  `stale_window_dropped` in the same colour as `auth_failed`. A dropped stale
  poll is routine and self-healing — the cloud returned `is_stale=True` and
  FHAI correctly refused to act on it. On a gateway where that happens six
  times in ten minutes the globe is permanently red, and a red icon that is
  always red hides the one that is not.

  Three severities now: `ok`, `warn` for transient states that usually clear on
  the next poll (`starting`, `retrying`, `stale_window_dropped`), and `danger`
  reserved for faults that will not — `auth_failed`, `no_credentials`,
  `circuit_breaker_tripped`, `error`.

- **The Ethernet icon warned about a cable that was never meant to be there.**
  It went amber whenever Ethernet was not the active link, so a site connected
  by WiFi was permanently warned about Ethernet. A link that is not carrying
  the site is now shown as idle; only the absence of *every* link is a warning.

  Both decisions moved into `$store.app.pollSeverity()` and `linkSeverity()`
  rather than staying as inline conditions in the markup, so "what counts as
  broken" has one answer.
...
- **The wizard confirms what hardware was found**, as a new second step:

  ```
  FHP   aGate X-01-AU   AGT-R1V1-AU
  2 aPowers · 27.2 kWh · 10 kW
  Solar ✓   Smart circuits ?   Generator —   aPBox ?
  ```

  A read of what registration already hydrated — `discover(tier=3)` writes the
  accessory flags and SKU into the gateway profile and the aPowers into the
  batteries table — not a second discovery. A second opinion about the hardware
  would be a second thing to be wrong.

  Shown as confirmation, not a choice: almost every site is one gateway with
  one or two aPowers, and approving a list you did not compile is a delay
  rather than a decision.

  A capability the cloud never reported renders **`?`**, distinct from a
  reported `—`. Saying "no solar" about a site that was never asked is worse
  than saying nothing, and it is the same rule the entity gating uses.

- **The gateway list shows the model and online state** discovery already
  returned and the wizard was discarding, so a gateway is identified by
  `aGate X-01-AU` rather than by eight digits of serial.

### Fixed

- `next()` capped at step 3, from when there were three steps. With five it
  would have stranded the user on the connection step with a Next button that
  did nothing.
...
- **The setup wizard now asks for the entity naming scheme**, before anything
  is published. Home Assistant assigns an entity id once, at creation, and
  keeps it — so choosing afterwards means renaming every entity and rewriting
  every dashboard that used the old ids. That is precisely what happened on a
  live site this month. One click here, or an afternoon later.

  Two choices, each previewed with a **rendered** entity id built from the real
  gateway serial rather than a template string:

  | Choice | Produces |
  |---|---|
  | Serial *(recommended)* | `sensor.franklinwh_00000042_grid_power` |
  | Model and serial | `sensor.franklinwh_agate_x_01_au_0042_grid_power` |

  The second exists for sites migrating from an earlier FranklinWH integration,
  whose dashboards already use that scheme. Neither offers `{gateway_name}`: a
  name is user-editable, so an id built on one moves when the gateway is
  renamed.

  Before a gateway is registered the preview uses the documented fixture serial
  rather than inventing one.

  The wizard already adapted to the deployment and still does — under the
  Supervisor the HA connection and MQTT broker configure themselves, so those
  questions are not asked. The prefix is asked in all three deployments,
  because MQTT discovery behaves identically in each.

  `POST /setup/prefix` delegates to the MQTT route that owns the validation
  rules rather than writing the config value itself.
...
- **`make_mock_serial()` / `is_mock_serial()`** — mock and demo gateways now
  have a defined serial format: numeric throughout, starting `999`, in both the
  serial and the short id.

  ```
  make_mock_serial(1)  →  99900000000099900001   short id 99900001
  ```

  Numeric because the short id reaches MQTT topics and entity ids, and Home
  Assistant forces entity ids to lowercase — a serial containing letters
  appears in two cases, `A02F` and `a02f`, which is hard to read and has
  already produced a duplicate device here. A numeric serial cannot have that
  problem. `999` because FranklinWH does not issue it, so demo data is
  recognisable at a glance in a topic, a log line or a support screenshot.

  An index outside 0–99999 is refused rather than truncated, which would give
  two mocks one serial.

### Changed

- The uppercase-serial convention now states that it is **not
  deployment-dependent**. Lowercase is forced by Home Assistant's entity-id
  format, not by add-on mode, so standalone is treated identically; MQTT
  topics, logs, the UI, the API and the database stay uppercase in both. A mode
  switch is explicitly ruled out — two behaviours is how one fact ends up
  spelled two ways, which is what the rule exists to prevent.
...
- **Serials are written UPPERCASE everywhere** — `99900000000099900001`, never
  `99900000000099900001` — now recorded as a convention in `CLAUDE.md` and
  guarded by `tests/test_serials_stay_uppercase.py`.

  A lower-cased serial is genuinely hard to read: the alpha run in the middle
  (`A02F` → `a02f`) stops standing out against the digits, and transcribing
  between the two forms goes wrong. It has already caused a real fault — an
  older build published `franklinwh_99900000000099900001` as a device
  identifier while the live one was uppercase, so Home Assistant carried two
  devices with 51 duplicate entities and the same display name.

  Two case-insensitive comparisons are deliberate and unchanged, because
  neither displays anything: `gateway_registry` matching an identifier, and
  `retained_cleanup` generating the lower-cased variant in order to *find*
  those ghosts.

  Home Assistant requires entity ids to be lowercase, so `{full_serial}` in the
  entity prefix template is the one unavoidable exception. It now logs a
  warning once per process explaining why, and both the guide and the token
  table mark it **avoid** in favour of `{short_id}` or `{model}`+`{serial4}`.
...
- **`docs/entity_naming.md`** — how entity ids are decided, what to use for a
  single gateway and for several, and how to take ids over from a previous
  integration.

  Written because the mechanism is genuinely easy to get wrong: `unique_id` is
  Home Assistant's internal key and has **no effect** on the entity id;
  `object_id` is what the id is built from; with neither, HA falls back to the
  device name — which is a user-editable label. Anchor on the serial, which
  never changes, not on a name, which does.

  The migration section is about history, not tidiness: long-term statistics
  are keyed on `entity_id`, so an entity that takes over an old id inherits its
  recorder history. It states the blocking rule plainly — two entities cannot
  hold one id, and **disabling** a config entry does not release its ids, only
  deleting it does — and answers per source: richo's HACS `franklin_wh` yes,
  FEM yes, a previous install of this add-on yes, a Local API or Modbus bridge
  *maybe* depending on how it publishes, and a cloud integration you must keep
  running no.

  The Local and Modbus Bridges turned out to deserve their own section rather
  than a "maybe": they publish MQTT discovery correctly — `unique_id` **and**
  `object_id` as `franklinwh_{node}_{key}`, one device per serial — so they are
  peers reading the same hardware by another route, not predecessors to migrate
  away from. Their entity ids **can** collide with FHAI's default prefix where
  a key matches a slug, which is a real trap; their state topics cannot. The
  guide says to give FHAI a distinct prefix, since the Bridge's is fixed in its
  source, and to let one source own each fact rather than letting Home
  Assistant pick a winner on every restart.

  Pinned by tests: every documented token must actually substitute, the worked
  examples must render exactly as printed, and the guide must keep naming the
  real rename endpoints.
...
- **Rename existing entities onto the current id scheme.** 0.6.59 made new
  entities carry the gateway in their id, but Home Assistant assigns an entity
  id once and keeps it — so a site that already had `sensor.fhp_state_of_charge`
  still had it, and a dashboard written against
  `sensor.franklinwh_agate_x_01_au_0091_*` still did not match.

  `GET /api/ha/entity-rename/plan` lists which of our entities would change and
  to what. `POST /api/ha/entity-rename/apply` performs it, and requires
  `confirm: "rename"` because Home Assistant does not rewrite YAML — anything
  referring to an old id has to be updated to match.

  No new machinery: the entity registry is writable over the same WebSocket
  `integration_conflicts` already authenticates against, and the target id is
  **derived, not guessed** — discovery publishes `object_id` equal to
  `unique_id`, so the id an entity would receive today is exactly
  `{domain}.{unique_id}`. Anything of ours that differs was created before the
  scheme it should be on.

  Only entities on the `mqtt` platform whose `unique_id` begins `franklinwh`
  are ever touched. Apply re-plans and intersects rather than trusting the
  request, so a stale list from an open browser tab cannot rename something
  that has since changed. Each rename is its own command: one refusal — a
  target id already held by a stale duplicate device is the expected one — does
...
- **Entity ids did not identify which gateway they belonged to.** FHAI
  published `unique_id` but not `object_id`. `unique_id` is Home Assistant's
  internal key and has no effect on the entity id — without `object_id`, HA
  builds the id from the *device name* plus the entity name, so a gateway
  labelled "FHP" produced `sensor.fhp_state_of_charge`. That identifies no
  gateway, collides on a multi-gateway site, and moves whenever the device is
  renamed — which happened three times in one release cycle.

  `object_id` is now published, rendered from the entity prefix template that
  already existed. Two new tokens let that template express the scheme a
  multi-gateway site is already using:

  | Token | Renders |
  |---|---|
  | `{model}` | `agate_x_01_au` — SKU stripped |
  | `{serial4}` | `0091` |

  So `franklinwh_{model}_{serial4}_` gives
  `sensor.franklinwh_agate_x_01_au_0091_circuit_1_power`, and the id no longer
  depends on the device label at all.

  **This does not rename entities that already exist.** Home Assistant assigns
  an entity id once, at creation, and keeps it. Existing `fhp_*` entities keep
  their ids until they are renamed in Home Assistant or the device is removed
  and rediscovered.
...
- **A TOU preset overwrote the schedule without backing it up, so
  "Stop / Restore" had nothing to restore.** The `Export to Grid (Always)`
  preset emulates a forced grid export — something Modbus does directly — by
  replacing the live TOU schedule with a single all-export tariff and switching
  mode. Stop / Restore is meant to put both back. The mode came back; the
  schedule did not, and the site was left on that one tariff.

  `_backup_tou_schedule()` was only ever called from cloud_dispatch's charge
  and discharge paths, and the preset path writes the schedule directly through
  `set_tou_schedule()`. So the preset destroyed the real schedule with nothing
  saved, and `_restore_tou_schedule()` returned `no_backup_file`.

  The preset path now backs up first. The existing 300-second staleness guard
  means a second preset applied straight afterwards cannot overwrite the backup
  with the already-overwritten schedule.

- **Stop / Restore reported success whatever happened.** It logged "dispatch
  stopped" regardless, so a restore that found no backup was indistinguishable
  from one that worked. It now reports the restored mode and the schedule
  restore result, and logs a warning — not an info line — when the schedule was
  not restored.

  **This does not recover a schedule already lost.** If the backup was never
  taken, there is nothing on disk to put back; the schedule has to be entered
  again, after which the next preset will save it.
...
- **Control Lock — a master enforcement switch with per-category locks.** FHAI's
  own UI puts Go Off-Grid behind a red modal: the utility grid and main panels
  remain live, the aGate is not a UPS, loads upstream lose power, and "I
  acknowledge the physical risks of forced islanding" must be ticked first.
  Home Assistant got the same command as a bare dropdown option that fired
  immediately.

  On 2026-09-20 it was selected while exploring options that appeared not to
  work — they were reverting for want of a state echo, fixed in 0.6.55 — and
  the grid relay opened, leaving the site in Outage for about three minutes.

  Four switches:

  | Switch | Default | Guards |
  |---|---|---|
  | Control Lock | on | master; off suspends everything for 300s, then re-arms |
  | Lock Grid Disconnect | **on** | `off_grid_mode → Off-Grid` |
  | Lock Operating Mode | off | `operating_mode` |
  | Lock Battery Dispatch | off | `dispatch_action → Charge/Discharge/Stop` |

  Only the grid is guarded out of the box. Locking mode or dispatch by default
  would break existing automations on upgrade, and neither physically endangers
  anyone; the grid relay does. Returning **to** grid is never blocked — the safe
  direction is always available.

  A refused command is logged and the entity is put back where it was, so the
  UI does not imply the command was accepted. The check runs before the command
  reaches the gateway, because checking afterwards is checking after the relay
  has opened.
...
- **The Operating Mode sensor and select spelled the same mode differently.**
  From a Home Assistant logbook export, both updating from the same poll at the
  same instant:

  ```
  12:17:26  sensor.fhp_operating_mode_2  'Time-Of-Use'
  12:17:26  select.fhp_operating_mode    'Time-of-Use'
  ```

  The normaliser matched values against an entity's declared options only for
  selects, and the sensor declared none — so it published the cloud's raw
  spelling. Any template, automation or dashboard comparing the two never
  matched, and nothing reported an error.

  Option matching now applies to any entity that declares options, and the
  sensor declares the same ones as the select. A value that matches no option
  is still passed through untouched rather than invented.
...
- **A changed dropdown snapped back to its old value.** None of the control
  entities are declared optimistic, so Home Assistant keeps showing the
  previous value until the state topic confirms the new one — and nothing
  confirmed it. `dispatch_command` applied the change and returned; the state
  topic was only written by the next poll, and polls are dropped whenever the
  cloud returns stale data, which happened 23 times in one session on a live
  gateway. A successful change therefore reverted in the UI and could stay
  wrong for minutes.

  A successful command now confirms itself on the state topic immediately. It
  is a confirmation, not a claim: it is sent only after the command succeeded,
  and the next real poll overwrites it if the hardware disagrees.

  The echo and the poll share one normaliser, so they cannot disagree about how
  a value is spelled — selects are matched case-insensitively against the
  entity's own options (the cloud says `Time-Of-Use`, Home Assistant accepts
  only `Time-of-Use`), and switches are `ON`/`OFF` rather than Python's
  `True`/`False`.


### Added

- **`docs/dashboard_entities.md`** — the entities grouped for building a
  dashboard: power-now sensors for flow cards, the `total_increasing` energy
  counters the Energy Dashboard requires and which slot each belongs in, and
  the component flows (solar→battery, grid→battery, battery→grid). Notes that
  Grid Power is signed and must not be split into import/export by hand — the
  two counters exist for that — and that ids ending `_2` mean a stale duplicate
  device still holds the un-suffixed id, so dashboards should point at the
  un-suffixed form once Home Assistant is restarted.

- `[BKL-CLOUD-GENERATOR-CIRCUITS]` in `docs/backlog.md`. `get_generator_detail`
  and `set_generator_charge_schedule` exist in the cloud library's **working
  copy only** — the pinned v0.4.9 has neither, and the working copy carries them
  under the same version string while `docker-compose.yml` mounts it over the
  installed package. A feature built against them would work in dev and fail in
  the add-on image. First step is a released, pinned version, not code.
...
- The device block, read back from Home Assistant's own MQTT diagnostics rather
  ...
  identifiers franklinwh_99900000000099900001
  ```
...
- **The Home Assistant device info panel now reads the way FEM's does**, which
  ...
  Serial number: 99900000000099900001
  ```
...
- The WebSocket return leg now confirmed end-to-end on a standalone Docker
  ...
  (ws://192.0.2.10:8123/api/websocket)` followed by `subscribed to
...
- **A 51-entity duplicate of the live gateway is now swept automatically.** A
  0.6.42 boot reported `left 'aGate 99900001'
  (franklinwh_99900000000099900001) in place — 51 entity config(s)`. That is
...
### Verified
- Endpoint returns 200 with real data for gateway 99900001. `winner_step_id="strategy_mixer"`, `matrix_count=16`.
...
- Live smoke on gateway 99900001 with `use_lp_optimizer=1`: `POST /api/smart_dispatch/eval/force` returned a real decision (HOLD / Max Price — Cease Charging) — no solver warnings.
...
- Live smoke: flipped `use_lp_optimizer=1` on gateway 99900001 → `POST /api/smart_dispatch/eval/force` returned a real decision (HOLD / Demand Charge Protection) with `LPOptimizer[99900001]: v0.5.0 scaffold — falling through to greedy (LP body ships in v0.5.1)` in the log.
...
- Live smoke: `POST /api/smart_dispatch/eval/force?gateway_id=99900001` returns a real decision (`HOLD`, `Demand Charge Protection`, `demand_window=True`) — the guard only triggers on the sentinel, not normal ticks.
...
- MicroTicker fires every 30 s on gateway 99900001.
...
- Post-restart live smoke: MicroTicker fires every 30 s on gateway 99900001, zero conflict warnings on subsequent ticks.
...
- MicroTicker still fires every 30s on gateway 99900001 (no regression from Phase 2.D)
...
- No behaviour change on my running container (Python 3.12 in Docker) — routes still register, MicroTicker still fires every 30s on gateway 99900001, /api/scheduler/jobs still returns 200 with 7 user automations.
...
- **After flag flip on gateway 99900001**: MicroTicker fires 20:17:04, 20:17:34, 20:18:04, 20:18:34, 20:19:04, 20:19:34 (every 30s exactly). Each fire runs full SD eval and produces `SmartDispatch [TICK] [99900001] action=HOLD rule='Demand Charge Protection' soc=69%`. Dedup gates prevent notification spam ("(same as prev — dedup)" on every tick).
...
- Live `POST /api/smart_dispatch/eval/force?gateway_id=99900001` returns a real decision — Meso → Greedy → `synthesize_and_optimize` chain confirmed.
...
- `MacroDiscovery.run_all()` manually invoked writes a full snapshot for gateway `99900001` (DNA, utility service, sd_config all populated).
...
- Live `POST /api/smart_dispatch/eval/force?gateway_id=99900001` returns a real decision (`HOLD` / `matrix_7` / Max Price rule) — StrategyMixer + context + evaluator all wired correctly through the package boundary.
