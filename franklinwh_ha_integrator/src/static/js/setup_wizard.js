...
function setupWizard() {
  return {
    ...
    async savePrefix() {
      // Delegates to /setup/prefix, which delegates again to the MQTT route
      // that owns the validation rules. Three copies of "what is a valid
      // template" is two too many.
      this.busy = true;
      this.error = null;
      try {
        const r = await fetch('api/setup/prefix', {
          method: 'POST', headers: {'Content-Type': 'application/json'},
          body: JSON.stringify({ template: this.prefixTemplate }),
        });
        if (!r.ok) {
          const d = await r.json().catch(() => ({}));
          throw new Error(d.detail || 'Could not save the naming scheme');
        }
        await this.refresh();
        this.step = 5;
      } catch (e) {
        this.error = e.message || 'Could not save the naming scheme';
      } finally {
        this.busy = false;
      }
    },
    ...
    async init() {
      await this.refresh();
      // Pre-select whatever is stored, so Continue is never a blind choice.
      this.prefixTemplate = this.state.steps?.prefix?.template || '';
    ...
    async next() {
      // Five steps now: gateway, hardware, connection, naming, finish. This
      // capped at 3 from when there were three, which would have stranded the
      // user on the connection step with a Next button that did nothing.
      if (this.step === 1) await this.refresh();
      this.step = Math.min(5, this.step + 1);
      // Both read state that registering a gateway has just changed.
      if (this.step === 2 || this.step === 3) await this.refresh();
    },
