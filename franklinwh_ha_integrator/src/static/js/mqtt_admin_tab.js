...
function mqttAdminTab() {
  return {
    ...
    // Home Assistant serves this panel in a sandboxed iframe, which blocks
    // window.prompt — and confirm() with it. Under ingress this returned
    // nothing and the Remove button silently did nothing at all, which on a
    // phone is indistinguishable from a broken button. CLAUDE.md already said
    // not to use prompt; confirm is the same restriction.
    askRemoveGhost(device) {
      this.ghostPending = device;
    },

    async removeGhost(device) {
      device = device || this.ghostPending;
  ...
  return {
    ...
    get preview() {
      ...
      return (this.draft || '')
        ...
        .replace('{short_id}', '99900001')
        .replace('{serial}', '99900000000099900001')
        .replace('{name}', 'FHP')
        .replace(/\s+/g, ' ')
        .trim();
    },
