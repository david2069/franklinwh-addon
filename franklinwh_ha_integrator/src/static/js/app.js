...
document.addEventListener('alpine:init', () => {
  ...
  Alpine.store('app', {
    ...
    /** Severity of the cloud poll: 'ok' | 'warn' | 'danger'.
     *
     * The globe was red for anything that was not running or ok, which put a
     * dropped stale poll — routine, self-healing, and the correct behaviour —
     * in the same colour as a failed login. On a gateway whose cloud returns
     * is_stale roughly every other poll, that is a permanently red icon that
     * means nothing, and a red icon that means nothing hides the one that
     * does.
     */
    /** Reopen the first-run wizard. Resets nothing. */
    async reopenSetup() {
      try {
        const r = await fetch('api/setup/reopen', { method: 'POST' });
        if (!r.ok) throw new Error('reopen failed');
        this.showSettings = false;
        // The wizard reads its own state on init, and it is mounted once at
        // page load — so a reload is the honest way to bring it back rather
        // than reaching into another component's internals.
        window.location.reload();
      } catch (e) {
        this.addToast?.('Could not reopen the setup wizard', 'error');
      }
    },

    pollSeverity(gw) {
      const status = gw?.poll_status;
      if (['running', 'ok'].includes(status)) return 'ok';
      // Transient and self-correcting: the next poll usually succeeds.
      if (['starting', 'retrying', 'stale_window_dropped'].includes(status)) return 'warn';
      return 'danger';
    },

    /** Human label for a poll status.
     *
     * The daemon chip used to test the status inline against five of the nine
     * values the poller actually sets, so `retrying`, `stale_window_dropped`,
     * `auth_failed`, `circuit_breaker_tripped` and `no_credentials` each
     * rendered a red chip with an icon and no text — the unexplained red dot
     * in the top bar, reported repeatedly. Every status gets a word here, and
     * the chip's colour comes from pollSeverity() so there is one rule.
     */
    pollLabel(gw) {
      const status = gw?.poll_status;
      return {
        ...
        starting: 'Starting',
        retrying: 'Retrying',
        // The cloud served a reading it had already marked stale, so the poll
        // was dropped rather than stored. Routine, and not a fault.
        stale_window_dropped: 'Waiting for fresh data',
        error: 'Comm Error',
        auth_failed: 'Sign-in failed',
        no_credentials: 'No credentials',
        circuit_breaker_tripped: 'Paused after errors',
        stopped: 'Offline',
        not_started: 'Offline',
      }[status] || (status ? String(status).replace(/_/g, ' ') : 'Unknown');
    },

    /** Severity of one network link: 'ok' | 'idle' | 'warn'.
     *
     * A site connected by WiFi is not in a warning state because Ethernet is
     * unplugged. Only the absence of *any* link is a problem.
     *
     * @param kind 1 Ethernet, 2 cellular, 3 WiFi — the gateway's own codes.
     */
    linkSeverity(gw, kind) {
      const active = gw?.network_connection;
      if (active === kind) return 'ok';
      const hasBackup = (kind === 3 && gw?.wifi_signal != null)
                     || (kind === 2 && gw?.mobile_signal != null);
      if (hasBackup) return 'ok';
      // Another link is carrying the site. Not this one's problem.
      return active ? 'idle' : 'warn';
    },
