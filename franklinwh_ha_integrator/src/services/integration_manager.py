...
def build_profile_from_snapshot(snapshot: dict, gw: dict | None = None,
    ...
    profile = {
        ...
        # ── The accessories themselves, as discovery names them ─────────
        # discover() returns AccessoryItem per registered accessory — serial,
        # type and the product name ("Smart Circuits V1-AU"). Only the has_*
        # booleans were kept, so every screen could say *whether* something was
        # fitted but never *what*, and the setup wizard showed four question
        # marks about a site whose accessories the cloud had already named.
        "accessory_items": [
            {
                "serial":    it.get("serial", ""),
                "type":      it.get("type_name", ""),
                "type_id":   it.get("accessory_type"),
                "name":      it.get("name", ""),
                "installed": it.get("create_time", ""),
            }
