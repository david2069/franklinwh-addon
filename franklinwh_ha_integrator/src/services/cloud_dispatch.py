...
class AsyncCloudDispatchService:
    ...
    async def backup_tou_schedule(self) -> None:
        """Back up the live TOU schedule before something overwrites it.

        Public because the TOU *preset* path overwrites the schedule too, and
        it does not go through this service. Applying a preset therefore
        destroyed the schedule with nothing saved, and the later
        "Stop / Restore" found no backup to put back — the schedule stayed as
        the single all-export tariff the preset had written.
        """
        await self._backup_tou_schedule()
