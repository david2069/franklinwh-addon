...
class MQTTPublisher:
    ...
    def _make_uid(self, short_id: str, slug: str, *,
        ...
        Supported tokens:
            ``{short_id}``      → last 8 chars of serial, e.g. ``24170091``
    ...
    def publish_discovery(self, full_serial: str, gateway_name: str, profile: dict,
        ...
        entities = get_entities_for_profile(profile)
        for ent in entities:
            disc_topic = (
    ...
    def publish_state(self, full_serial: str, stats: dict) -> None:
        ...
        for ent in AGATE_ENTITIES:
            ...
            if ent.ha_type in ("binary_sensor", "switch"):
                value = "ON" if str(value).lower() in ("true", "1", "on") else "OFF"
            # DEF-SELECT-NORM: Normalise select values against defined options.
            # franklinwh-cloud returns 'Time-Of-Use' (capital O); our entity options
            # have 'Time-of-Use' (lowercase o). HA rejects non-matching values.
            # Case-insensitive match → emit canonical form from EntityDef.options.
            if ent.ha_type == "select" and ent.options and value is not None:
                _val_lower = str(value).lower()
                _match = next((o for o in ent.options if o.lower() == _val_lower), None)
                if _match:
                    value = _match
            # DEF-ENUM-EMPTY: Skip publishing empty-string values for enum sensors.
            # HA device_class='enum' requires the value to be one of the defined options.
            # When not in TOU mode, tariff sensors legitimately have no value — don't publish ''.
    ...
    def _build_entity_payload(
        ...
        uid = self._make_uid(serial, ent.slug)
        payload = {
            "unique_id": uid,
    ...
    def tombstone_discovery(self, ha_type: str, unique_id: str) -> None:
        ...
        Args:
            ...
            unique_id: Full unique_id of the entity (e.g. 'franklinwh_24170091_grid_status')
