...
async def _hydrate_gateway_fields(gw: dict, registry) -> dict:
    ...
    # Kept outside the branch below: what is installed does not depend on
    # whether the poller happens to be running, and during first-run setup it
    # usually is not.
    last_data: dict = {}
    ...
    # Accessories, from the profile the cloud wrote at registration plus any
    # live evidence. This used to sit inside `if live:`, so a gateway that was
    # not polling — every gateway during first-run setup — reported no
    # accessories at all, however much the profile knew.
    # The flags are already on `gw`, hydrated from the profile at the top of
    # this function — which then pops `profile_json`. The previous derivation
    # re-read that popped column, so it parsed "{}" every time and the profile
    # contributed nothing; only `last_data` ever produced an accessory.
    from src.services.accessories import names as _accessory_names

    raw_acc = last_data.get("accessories") or _accessory_names(gw, last_data)
    if gw.get("grid_type") == "off_grid":
        gw["accessories"] = [a for a in raw_acc if a != "mac1"]
    else:
        gw["accessories"] = list(raw_acc)
