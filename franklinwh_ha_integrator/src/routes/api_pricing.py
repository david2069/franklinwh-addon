...
# ── Tariff bundles: export, import, share ────────────────────────────────────
#
# A tariff is entered by hand from a bill and is easy to lose — an add-on
# uninstall can take /data with it, backups keep seven days, and the companion
# Bridges hold none of this. A file the user keeps outside the add-on is the
# only thing that survives all three.

class TariffImportRequest(BaseModel):
    bundle: dict
    replace: bool = False


@router.get("/pricing/tariffs/export")
async def export_tariff_bundle(presets: bool = True):
    """Download the tariffs, utility services and presets as one JSON bundle.

    Account identifiers — NMI, account number, meter serial — are stripped, so
    the file is safe to hand to somebody on the same retailer plan.
    """
    from fastapi.responses import JSONResponse

    from src.services.tariff_bundle import export_bundle

    bundle = await export_bundle(include_presets=presets)
    return JSONResponse(
        content=bundle,
        headers={"Content-Disposition": 'attachment; filename="fhai-tariffs.json"'},
    )


@router.post("/pricing/tariffs/import")
async def import_tariff_bundle(req: TariffImportRequest):
    """Restore a bundle produced by the export above.

    Merges by primary key by default, so re-importing your own export changes
    nothing. `replace: true` empties the tables first — offered because a
    half-corrected tariff is harder to reason about than a replaced one.
    """
    from src.services.tariff_bundle import import_bundle

    try:
        result = await import_bundle(req.bundle, replace=req.replace)
